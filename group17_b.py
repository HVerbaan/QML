...
for i in N:
    for v in V:
        alpha[i,v] = m.addVar(vtype=GRB.CONTINUOUS, lb=0, name="alpha_%s,%s" %(i,v))

# Variable 6 - Time spent charging at node i by vehicle v
beta = {}
for i in N:
    for v in V:
        beta[i,v] = m.addVar(vtype=GRB.CONTINUOUS, lb=0, ub=maximum_battery, name="beta_%s,%s" %(i,v))
...
for v in V:
    m.addConstr(
        ...
        name=f"start_battery_{v}")

# Constraint 12 - Battery updating after traveling and charging
for v in V:
    for i in N:
        for j in N:
            if i != j:
        ...
        for j in N:
            if i != j:
                m.addConstr(
                    ...
                    name=f"travel_feasible_at_arc_{i}_to_{j}_by_{v}")
...
# Constraint 17 - Updating load value after picking up a load
for v in V:
    for j in N:
        for i in N:
            if i != j:
...
if m.status == GRB.OPTIMAL or m.status == GRB.SUBOPTIMAL:
    ...
    print("No feasible solution. Status:", m.status)
