...
if case == 1:
    ...
    discharge = 0.7                             # Discharging rate of the battery in vehicle
    ...
    K = 4                                       # Amount of vehicles in fleet
    maximum_loading = 200                       # Maximum loading capacity of vehicle v (currently single type)
...
# Constraint 1
routing_cost = quicksum(distance[i,j] * x[i,j,v] for i in N for j in N for v in V if i != j)
charging_cost = quicksum(period_cost[p] * ct[i, v, p] for i in N for v in V for p in P)
...
#    temp_time = []
    ...
    # for i in route_nodes:
    #     temp_battery.append(rb[i,v].X)
    #     temp_time.append(a[i,v].X)
    #     print(a[i,v].X)
    # y_battery.append(temp_battery)
    # x_time.append(temp_time)
        
#array_battery = np.array(y_battery)
#battery_per_vehicle = array_battery.reshape(-1,4).T

#array_time = np.array(x_time)
#time_per_vehicle = array_time.reshape(-1,4).T

#for v in V:
#    plt.plot(time_per_vehicle[v], battery_per_vehicle[v])
#plt.xlabel("Time")
#plt.ylabel("Battery level")        
#plt.title("Change in battery level")
#plt.show()

#for v in V
#plt.plot(x_time, y_battery)
#plt.xlabel("Time")
#plt.ylabel("Battery level")        
#plt.title("Change in battery level")
#plt.show()
...
# for list_value in y_battery:
#       x_value_list = range(1, len(list_value)+1)
#       plt.plot(x_value_list, list_value)
# plt.show()
...
#     for i in route_nodes:
#         y_battery.append(rb[i,v].X)
#         print(y_battery)
#         x_time.append(a[i,v].X)
...
# for v in V:
#     for i in N:
#         y_load_cap = y_load_cap.append(cl[i,v].X)
#         x_time = x_time.append(a[i,v].X)

# plt.plot(arr_time, batt_level)
# plt.xlabel("Time")
